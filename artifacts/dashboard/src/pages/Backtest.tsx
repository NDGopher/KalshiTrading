...
import { FlaskConical, Play, BarChart3, Target, ChevronDown, ChevronRight, TrendingUp, Crosshair, Award, ChevronLeft, Database, Download, CheckCircle, AlertTriangle, ExternalLink } from "lucide-react";
import {
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div>
              ...
              <a
                ...
                className="text-[10px] font-mono text-muted-foreground hover:text-primary flex items-center gap-0.5 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                {trade.kalshiTicker.length > 20 ? trade.kalshiTicker.slice(0, 20) + "…" : trade.kalshiTicker}
...
export default function Backtest() {
  ...
  function selectRun(id: number) {
    setSelectedRunId(id);
    setTradePage(0);
    setTimeout(() => tradesRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
  }
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Real settled Kalshi market data — NFL spreads, La Liga, Serie A, UCL, NBA playoff series. Real historical outcomes. Simulated model probabilities.
          </p>
        </div>

        {/* Honest disclaimer about what is real vs simulated */}
        <div className="rounded-lg border border-yellow-500/40 bg-yellow-500/8 p-4 flex gap-3">
          <AlertTriangle className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
          <div className="space-y-1">
            <div className="text-sm font-semibold text-yellow-300">Simulated Model Probabilities — Not Real Claude AI</div>
            <div className="text-xs text-yellow-200/70 leading-relaxed">
              <span className="font-medium text-yellow-200">What is real:</span> Market tickers, expiry dates, prices at time of entry, and final settlement outcomes are all genuine historical Kalshi data.{" "}
              <span className="font-medium text-yellow-200">What is simulated:</span> The "AI model probability" is generated by a mathematical formula targeting ~57% directional accuracy — it is <span className="font-semibold">not real Claude Haiku analysis</span>.
              The P&amp;L figures show what would have happened <em>if</em> such a model existed.
              Enable "Use AI analysis" to run an external model on each historical market (slow, optional; generates model probabilities when configured).
            </div>
          </div>
        </div>
          ...
          <Card className="glass-panel border-white/10">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm text-muted-foreground">Use external AI analysis (optional)</label>
              </div>
          ...
          <Card className="glass-panel border-white/10">
            <CardHeader className="border-b border-white/5 bg-black/20 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>{trades.length} trades — using simulated model probabilities (57% accuracy target)</CardDescription>
              </div>
              {totalPages > 1 && (
                  ...
                  <tbody className="divide-y divide-white/5">
                    {runs.map((r) => (
                      <tr key={r.id} className={`hover:bg-white/5 transition-colors ${selectedRunId === r.id ? "bg-primary/5" : ""}`}>
                        ...
                        <td className="p-3">
                          <Button variant="ghost" size="sm" onClick={() => selectRun(r.id)} className="text-primary hover:text-primary/80">View Trades</Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
